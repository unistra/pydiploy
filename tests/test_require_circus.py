...
from pydiploy.require.circus import app_circus_conf, app_reload, circus_pkg, upstart
...
class CircusCheck(TestCase):
    ...
    @patch('fabtools.system.distrib_release', return_value='16.04')
    ...
    def test_circus_pkg_old(
        ...
        circus_pkg()
        self.assertEqual(
            python_install.call_args_list[-3:],
            [
                call('circus', use_sudo=True, upgrade=False),
        ...
        circus_pkg()
        self.assertEqual(
            python_install.call_args_list[-4:],
            [
                call('circus', use_sudo=True, upgrade=False),
                call('circus-web', use_sudo=True, upgrade=False),
                call('gevent', use_sudo=True, upgrade=False),
    ...
    @patch('fabtools.system.distrib_id', return_value='Ubuntu')
    @patch('fabtools.system.distrib_release', return_value='18.04')
    @patch('fabtools.require.deb.packages', return_value=Mock())
    @patch('fabtools.require.deb.ppa', return_value=Mock())
    @patch('fabtools.require.python.install', return_value=Mock())
    @patch('fabtools.files.upload_template', return_value=Mock())
    ...
    def test_app_reload_systemd(self, api_sudo, api_settings, is_systemd):
        ...
        self.assertEqual(
            api_sudo.call_args_list,
            [
                call('systemctl is-active circus.service'),
                call('circusctl reloadconfig'),
                # call('circusctl restart application_name'),
            ],
        )
