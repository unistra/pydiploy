...
def python_pkg(update=False):
    ...
    if fabtools.system.distrib_id() == 'Ubuntu' and float(fabtools.system.distrib_release()) >= 18.04:
        if env.remote_python_version < 3:
            fabric.api.sudo('add-apt-repository universe')

        fabtools.require.deb.packages(
            [
                'python2-dev' if env.remote_python_version < 3 else 'python%s-dev' % env.remote_python_version,
        ...
        fabtools.require.deb.packages(
            [
                'python-dev' if env.remote_python_version < 3 else 'python%s-dev' % env.remote_python_version,
                'python-pip',
            ],
            update=update,
        )
        ...
        with fabric.api.cd(env.remote_current_path):
            ...
            with shell("HOME=~%s %s" % (env.remote_owner, env.shell)):
                ...
                fabtools.require.python.install(
                    'pip', upgrade=True, use_sudo=True, user=env.remote_owner, pip_cmd=pip_cmd, quiet=True
                )
                ...
                fabric.api.sudo('pip install --log-file %s --quiet -e .' % pip_log, user=env.remote_owner, pty=False)
